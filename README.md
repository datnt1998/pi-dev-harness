...
- `safe-ops` — blocks model writes to `.env`, `.git/**`, `node_modules/**`; confirms destructive model shell commands in TUI and blocks them headlessly. Guardrail, not sandbox. Configuration: no configuration file; the blocklist is fixed in `lib/safe-ops-core.ts`. Known limitations: path-based blocklist only — a creative path that resolves into a protected file is out of scope; destructive-command confirmation relies on pattern matching and may miss novel command shapes.
- `ticket-runner` — `/implement-all`, `/implementation-status [--verbose]`, `/implement-all-stop`; tools `batch_next`, `batch_report`, `batch_worker_lane`; persisted state, source fingerprint, retries, continuation cap, and an optional generic pilot ledger. Configuration: no settings file; batch inputs come from `.scratch/<effort>/` ticket artifacts and `/implement-all` arguments. Known limitations: state persists as append-only session-log entries, so a corrupt newest snapshot degrades to a partial rebuild rather than erroring; batch state is single-writer by design and parallel writers require the worktree lease protocol.
- `harness-tui` — brandless responsive footer preserving context %, session cost, cwd/path, git branch, thinking level, and model at every width; `/harness-tui status|on|off`. Identity-free (label defaults to the folder name, no theme/branding). **One footer owner:** a project shipping its own footer must set `.pi/harness-tui.json` `{ "enabled": false }` or run `/harness-tui off` so the two never fight. Pure layout/format logic in `lib/tui-core.ts`. Known limitations: usage figures depend on the shared provider-quota authority and are as fresh as its cadence; a captured command ctx goes stale after session replacement and handlers no-op rather than fail loud.
- `provider-usage` — standalone belowEditor view of the same machine-readable quota snapshot used by the launch gate (the `harness-tui` footer consumes that snapshot too, so this widget is **default OFF**). It shows short/weekly capacity, freshness, band, protected balances, and gate reason. `/provider-quota status|on|off|refresh|snapshot`; enable via `.pi/provider-usage.json` `{ "enabled": true }`. Select a session-stable quota source independently of the active main model with `.pi/quota-gate.json` `{ "provider": "<supported-provider>", "providerIdentity": "<stable-account-key>" }`; without it, UI-only compatibility freezes the supported active provider at session start. **Trust/network:** OAuth is read only by the existing provider fetch adapter; credentials are never returned or persisted. Known limitations: only the supported providers are queryable and the snapshot is a best-effort display — the widget never gates launches; unsupported providers freeze the fallback for the whole session.
...
- `autocompact` — proactive context compaction ahead of Pi's overflow safety net: tiered warnings + a "tokens left" indicator, compacts at a configurable trigger (percent AND/OR absolute token cap, effective = min) at safe idle boundaries. `/autocompact status|on|off|at <pct|tokens>|warn <pct>|focus <text|clear>|native on|off|reorient on|off|now`. Layered settings: global `$PI_CODING_AGENT_DIR/autocompact.json` + project `.pi/autocompact.json` (wins). Pure logic in `lib/autocompact-core.ts`.
  ...
  - **Mid-run (long single runs):** our own `ctx.compact()` aborts the active run, so it can only fire at idle. Optional `/autocompact native on` aligns Pi's native between-turns compaction (`compaction.reserveTokens`) with the trigger by writing the **project** `.pi/settings.json` only. This is **off by default**, so reloads preserve an explicitly configured project reserve; child subagents never rewrite it. Non-interrupting; applies from the next session/reload (Pi has no runtime setter for `reserveTokens`). `/autocompact native off` restores fixed-reserve ownership; `/autocompact status` shows the mode.
  ...
  - Known limitations: extension compaction can only fire at idle (mid-run compaction would abort the run); re-orientation is lazy advisory text, not a guaranteed context restore; `native on` rewrites the project `.pi/settings.json` reserve and relies on reload to take effect.
- `review-graph` — prints a compact navigation map for a change set so reviewer briefs carry the codebase context instead of each reviewer re-deriving it: per changed file the export-surface delta, direct dependents (one hop of importers, plus a capped second hop), and guard files (test dependents at one or two hops), with workspace-package (monorepo) import resolution and an explicit `unresolved` section where static resolution cannot see. `/review-map [base..head]`; with no argument it maps the dirty tree (unstaged + staged + untracked source files). Run it when composing a reviewer brief — after the boundary commit — and read the summary line before pasting: plant the block only when it shows a wide dependent surface, since for a narrow change the paste is overhead rather than navigation; after a fix round, re-run it on the fix-round range to scope the re-review to what actually changed. TS/JS surfaces only; outside them the command degrades to a clear message. The map is advisory navigation, never evidence — findings still cite real files (review discipline: `skills/engineering-workflow/references/code-review.md`). Pure graph logic in `lib/review-graph.ts`. Configuration: no configuration file; the base range comes from the command argument. Known limitations: static import resolution only — dynamic imports and non-TS/JS files are reported as unresolved, and the graph is a one-to-two-hop approximation, not a full dependency closure.
- `run-track` — operator-gated evidence recording and guardrail acknowledgment for batch runs; tools `run_track_record_evidence`, `/run-track-ack`. Configuration: no configuration file; tracks are scoped to the current session branch. Known limitations: metadata-only events (no raw content payloads); deduplication is by action/facts digest, so two semantically different occurrences with identical facts are collapsed.
- `session-gc` — reclaims disk from finished agent work across three scan areas: the host session store's directory for the current project, the subagent temp root, and the project-local `.pi-subagents/artifacts`. `/gc status|dry-run|run|auto on|off|days <sessionsDays> <artifactsDays>`; `dry-run` before `run` is the safe default flow. Protection is fail-closed: the current session, a non-terminal run, anything newer than the retention window, or an unrecognized layout is never a deletion candidate. Defaults: sessions retained thirty days, artifacts retained seven days, auto sweep off (an enabled auto sweep runs at most once a day and reports only when it reclaimed something). Deleting a session file removes its resume history — run `dry-run` first to review candidates. The sessions area targets only the directory whose name the host derives from the current project path; when that directory cannot be located the area is skipped, and other projects' session stores are never scanned. Pure planning logic in `lib/gc-core.ts`. Configuration: layered settings in global `$PI_CODING_AGENT_DIR/session-gc.json` and project `.pi/session-gc.json` (project wins entirely when present); fields `sessionsDays`, `artifactsDays`, `auto`, `lastSweepMs`. Known limitations: retention is age-based only — there is no per-session pin, and unknown run-state values are treated as active (never deleted), which can keep stale artifacts alive.

### Skills (19)

`pi-harness`, `engineering-workflow`, `codebase-design`, `domain-modeling`, `git-rules`, `repo-hygiene`, `release-versioning`, `release-check`, `ticket-readiness`, `batch-implementation`, `prototype`, `wayfinder`, `memory-management`, `react-best-practices`, `react-doctor`, `make-interfaces-feel-better`, `diagram-design`, `tldraw-diagrams`, `reasoning-discipline`.

`diagram-design` is vendored from [cathrynlavery/diagram-design](https://github.com/cathrynlavery/diagram-design) under MIT and creates editorial, self-contained HTML files with inline SVG across 27 technical/product diagram types. Its bundled `LICENSE`, `THIRD_PARTY_LICENSES.md`, and `UPSTREAM.md` preserve upstream attribution and the pinned source commit. Brand onboarding writes only to the consumer repository's `.pi/diagram-design/style-guide.md`; package-installed files stay immutable. `/diagram` selects this file-based path by default; use tldraw only when the request explicitly needs an editable canvas.
...
| `diagram-design`, `diagram` | none for standalone HTML/SVG generation; optional browser for preview | produce a self-contained `.html`; export PNG/SVG only when requested |
| `tldraw-diagrams`, editable canvas requests | tldraw offline app + its `~/skills/tldraw-offline` operator skill | prompt to install tldraw offline, else fall back to an ASCII sketch or `diagram-design` HTML when acceptable |
