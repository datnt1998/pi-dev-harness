...
- `provider-usage` — standalone belowEditor widget showing AI provider quota (Claude 5h/Week, Codex windows) with reset timers, in any project; `/provider-quota status|on|off|refresh`. **Trust/network:** reads the active provider's OAuth token from the local Pi auth store and calls the provider usage endpoint (no secret stored/logged; Claude/Codex only). Opt out per project via `.pi/provider-usage.json` `{ "enabled": false }`. Pure parse/format logic in `lib/provider-usage-core.ts`.
