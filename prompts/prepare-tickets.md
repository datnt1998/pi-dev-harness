...
4. Follow `references/scratch-organization.md`: reuse the ticket source's registered effort when possible; otherwise create/reuse `.scratch/<batch>/` with `index.md`. Write and link `execution-manifest.md` (with the fingerprint header) and `readiness-report.md` from that registry.
