...
function leaseCoaching(resumed: boolean): string {
  ...
  return `${lead} Implement and parent-validate under it, close the stable writer handoff and confirm review_allowed, collect separate fresh sealed Falsification and Adversarial-authority evidence (with any required degradation acknowledgment), parent-dispose findings, at most one bounded fix-worker round, then batch_report with evidence matching the lease lifecycle.`;
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("agent_settled", (_event, ctx) => {
    ...
    pi.sendUserMessage(
      "/skill:batch-implementation continue the active ticket batch: call batch_next, record eligibility, acquire/close the exclusive writer lease through batch_writer_lease, implement one ticket, collect separate sealed Falsification and Adversarial-authority review axes only after review_allowed, parent-dispose findings, allow at most one bounded fix-worker round, then batch_report.",
      { deliverAs: "followUp" },
    );
  });
  ...
  pi.registerCommand("implement-all", {
    ...
    handler: async (args, ctx) => {
      ...
      pi.sendUserMessage(
        `/skill:batch-implementation A pre-approved ticket batch is active (source ${sourcePath}, commit=${commit}). Follow that skill's protocol exactly; the gates in brief: explicit parent eligibility, one exclusive parent-writer lease per worktree via batch_writer_lease (worker lane only for frozen, reversible, explicitly scoped pilot work with a parent-known falsifiable bar); close the writer handoff and record one stable implementation fingerprint, then batch_writer_lease action review_allowed — reviewers never acquire mutation authority. Require separate fresh sealed Falsification and Adversarial-authority reviews against that fingerprint, with actual provider/model, fallback, effective-model, and effective-thinking provenance recorded before each affected stage; on degraded topology require an explicit operator continue acknowledgment and never call the result independent. If pi-subagents is available, use it only within the exclusive lease and never as a second concurrent writer; if unavailable, a structured self-review fallback may aid diagnosis but cannot authorize high-risk completion or independence. Missing, combined, self, stale, unsealed, or mutated reviews fail closed. Parent-dispose findings; permit at most one bounded fix-worker round, then revalidate and focused re-review when substantial — a second substantial fix or repeated finding escalates. Parent implementation after delegation must be explicit rework/strong-route evidence. Call batch_report with evidence matching the recorded lease lifecycle — self-asserted closed leases are rejected. Continue until batch_next reports done or a decision is required; do not ask for per-ticket confirmation.`,
