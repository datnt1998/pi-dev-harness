...
The protocol's moves, applied to the domain where fluency most effectively masks
emptiness: prose. Writing always reads well to whatever produced it — fluent ≠ true has a
sibling, fluent ≠ good. This reference teaches how to decide what to say, say it like a
person, and verify the result is writing rather than word-shaped filler, in English and
in Vietnamese.

## Boundary with response-shape.md

`engineering-workflow/references/response-shape.md` is the single normative source for
the final shape of user-facing chat prose in this harness — answer-first, no
preamble/closer, exceptions never capped. This reference governs prose craft during
drafting and review — register, concreteness, rhythm, slop — for any prose deliverable,
chat or otherwise. It never overrides the shape contract: when a chat response is being
shaped, `response-shape.md` wins on structure; this reference still governs the sentences
inside it.

## When to Load This Reference

Load before drafting — not as a polish step — whenever the deliverable is prose a human
will read: docs, READMEs, posts, marketing and landing copy, emails, reports, UX
...
## Known Failure Modes (writing-domain instances)

The writing-domain form of the failure modes in `SKILL.md`:

- **Fluency inflation** — silence gets filled with words that could open any text and
  therefore open none. An intro that fits every topic carries zero information about
  this one. Confidence in the prose rises with token count, not with content.
...
- **Throat-clearing** — restating the question, announcing what will be said, hedging
  before saying it, summarizing what was just said. The reader wanted the middle part.
- **Uniform register** — one corporate-neutral voice for a changelog, a personal note,
  and an outage apology. Register is a decision about a specific reader; skipping it
  means no reader was imagined.
...
- **Translationese** — generating Vietnamese as relexified English (or any target
  language through a source-language template): source-language sentence shapes, calqued
  idioms, borrowed punctuation and capitalization conventions. Grammatical, and instantly
  foreign.
...
2. **Find the core claim before drafting.** State in one sentence the thing this piece
   exists to say. If that sentence cannot be produced, drafting is not ready to start —
   it would generate word-shaped filler around an absence. Every section must serve that
   claim.
3. **Spend a specificity budget.** Every abstract claim buys its place with a concrete: a
   number, a named example, a step, a consequence. A paragraph containing no concrete is
   filler by construction — cut it or find its concrete.
4. **Write one person to one person.** Draft as if saying it to the specific reader from
   step 1. Sentences that would never be said aloud to a person are sentences a person
   will not want to read.
5. **Subtract before delivering.** One deliberate pass to delete: intensifiers doing no
   work, hedges on things already verified, repeated points, the summary of what the
   reader just read. What survives is the writing.
...
- **Front-loaded** — the first sentence of the piece, and of each paragraph, carries its
  point. A reader who stops anywhere leaves with the most important part reached.
...
- **Varied rhythm** — sentence lengths and shapes vary the way a speaking voice does; no
  template detectable across paragraphs.
...
## Slop Catalog — English (matches are failed gates)

- Openers that fit any topic: "In today's fast-paced world…", "In the ever-evolving
  landscape of…", "Let's dive in".
- Filler-lexicon words: delve, tapestry, landscape, unlock, elevate, empower, seamless,
  robust, leverage-as-verb, game-changer, cutting-edge, "rich" anything.
...
- Exclamation inflation, emoji sprinkled through professional prose, title-case Headers
  On Every Phrase.
- Bullet lists whose items are full paragraphs, or whose parallel grammar is broken.

## Slop Catalog — Vietnamese
...
- Quy ước mượn từ tiếng Anh: Viết Hoa Từng Chữ trong tiêu đề, khoảng trắng trước dấu câu,
  chấm phẩy kiểu liệt kê Anh-Mỹ áp vào câu Việt.
...
- Titles and subject lines are read far more than the body — they get the specificity
  budget first, not last.
- What the piece displaces: the reader's time. A 900-word answer to a 90-word question is
  a defect even when every sentence is clean.

## Verify (fluency makes this mandatory — prose always passes its own re-read)

Apply Harness Leverage from `SKILL.md`: anything a granted capability can check must be
checked with it, as a loop, until a full pass over the final text is clean.

1. **Read-aloud pass.** Sentence by sentence: would this be said, in this voice, to the
...
2. **Slop scan.** Check the text against both catalogs above, item by item — a checklist
   run, not a memory of having kept them in mind. Use search when the environment grants
   it.
3. **Deletion test.** Per paragraph: remove it; did the reader lose anything needed? Keep
   only survivors.
...
5. **Register check.** English: one voice throughout. Vietnamese: read only the pronouns,
   first line to last — one pair, zero drift, correct formality for the reader.
...
| Pad with filler lexicon and triads | Spend concretes: numbers, names, steps, consequences |
| Trust a re-read of the same prose | Run the itemized scans — fluency always passes itself |
...
| Answer at the length that came out | Answer at the length the question deserved |
