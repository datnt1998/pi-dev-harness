...
- **"What should this look like?"** → read `UI.md` (same directory). Generate several radically different UI variations on one route, switchable via a URL search param and a floating bottom bar. Follow the repository's framework/routing conventions; pair with `/skill:make-interfaces-feel-better` when polishing the winner.
