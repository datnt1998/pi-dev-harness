---
name: tldraw-offline
description: Operate the user's tldraw offline canvas app, including open .tldraw or .tldr files. Use whenever a task involves inspecting, editing, arranging, connecting, linting, or scripting a tldraw Desktop canvas, drawing a diagram/flowchart/architecture/workflow, or turning an ASCII/textual sketch into a real tldraw drawing. Install tldraw offline first if it is not present.
---
...
## Install if missing (harness note)

This is a **vendored copy** of the third-party tldraw offline operator skill
(proprietary; "not open source, all rights reserved" — see `README.md`). The
package ships the instructions; the running desktop app provides the server and
the `tq` helper. Do not treat this copy as owned by the harness.

Before operating, ensure the app is installed and running:

- Ready when `~/Library/Application Support/tldraw/server.json` exists (macOS;
  Windows/Linux differ) and its `port` responds.
- If not installed, install **tldraw offline** from `https://offline.tldraw.com`
  (or `github.com/tldraw/tldraw-offline/releases/latest`; a `brew install --cask`
  may also exist — verify before relying on it), launch it, enable agent access,
  and open a document. **Do not fabricate install steps** — if it cannot be
  confirmed, ask the user, then continue once the server responds.
- The `tq` helper ships alongside this skill and the app also installs it at
  `~/skills/tldraw-offline/tq`; use either, or the raw `curl` forms below.

## Server

The default server is `http://localhost:7236`. If that port is not active, read the `port` from `~/Library/Application Support/tldraw/server.json`.
